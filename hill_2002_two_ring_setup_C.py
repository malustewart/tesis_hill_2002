...
class SetupDescription:
    ...
    coupler_1_T_from_ext_laser_dB: float = np.nan
    coupler_1_T_from_ring_laser_dB: float = np.nan
...
class SingleRunResults:
    ...
    ring_laser_1_power_mW : np.ndarray
    ring_laser_2_power_mW : np.ndarray
    ext_laser_power_mW : np.ndarray
    scope_fig : Figure
...
def save_result_metrics_csv(
    ...
    def flatten_and_filter_result(result):
        ...
        # run_param fields:
        for k, v in vars(r.run_params).items():
